...
%token NUM ID BEG END READ WRITE IF THEN ELSE ENDIF WHILE DO ENDWHILE L G LE GE NE EE REPEAT UNTIL BREAK CONTINUE DECL ENDDECL INT STR MES
%left '+' '-'
%left '*' '/' '%'
%right '='
...
OutputStmt : WRITE '(' expr ')' ';' {char *c="write";
	...
	| WRITE '(' MES ')' ';' {char *c="write";
				$$=makeOperatorNode(c,2,$3,NULL,NULL);}
	;
AsgStmt : ID '=' expr ';' {char *c="=";
$$=makeOperatorNode(c,3,$1,NULL,$3);}
	| ID '=' MES ';' {char *c="=";$$=makeOperatorNode(c,3,$1,NULL,$3);}
	;
