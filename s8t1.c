...
else if(temp->nodetype==37)
	{
	int s1,s2,p;
	if(temp->left==NULL || temp->right==NULL || GLookup(temp->left->varname)==NULL || GLookup(temp->right->varname)==NULL)
		{
		printf("Invalid operation found\n");
		exit(1);
		}
	s1=GLookup(temp->left->varname)->binding;
	s2=GLookup(temp->right->varname)->binding;
	p=getReg();
...
if(TLookup(name)!=NULL)
	{
	printf("The same variable name for user type is given\n");
	exit(1);
	}
...
if(new==NULL)
	...
	if(check_par_type(new->paramlist,Paramlist))
		...
		new->flabel=fl;
		mc--;
		if(mcheck[new->funcposition]>0)
			{
			printf("function redeclared\n");
			exit(1);
			}
		mcheck[new->funcposition]++;
